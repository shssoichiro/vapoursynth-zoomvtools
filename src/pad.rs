...
pub fn pad_reference_frame<T: Pixel>(
    ...
    assert!(dest.len() >= max_offset + ref_pitch.get() * (vpad - 1) + hpad);
    ...
    unsafe {
        ...
        pad_corner(
            ...
            *dest.get_unchecked(pfoff + width.get() - 1),
            hpad,
            vpad,
            ref_pitch,
            dest,
        ...
        pad_corner(
            ...
            *dest.get_unchecked(pfoff + (height.get() - 1) * ref_pitch.get()),
            hpad,
            vpad,
            ref_pitch,
            dest,
        ...
        pad_corner(
            ...
            *dest.get_unchecked(pfoff + (height.get() - 1) * ref_pitch.get() + width.get() - 1),
            hpad,
            vpad,
            ref_pitch,
            dest,
        ...
        for i in 0..width.get() {
            let value = *dest.get_unchecked(pfoff + i);
            let mut poff = offset + hpad + i;
            for _j in 0..vpad {
                *dest.get_unchecked_mut(poff) = value;
                poff += ref_pitch.get();
            }
        }
        ...
        for i in 0..height.get() {
            let value = *dest.get_unchecked(pfoff + i * ref_pitch.get());
            let poff = offset + (vpad + i) * ref_pitch.get();
            dest.get_unchecked_mut(poff..)
                .get_unchecked_mut(..hpad)
                .fill(value);
        }

        // Right
        for i in 0..height.get() {
            let value = *dest.get_unchecked(pfoff + i * ref_pitch.get() + width.get() - 1);
        ...
        // Down
        for i in 0..width.get() {
            let value = *dest.get_unchecked(pfoff + i + (height.get() - 1) * ref_pitch.get());
